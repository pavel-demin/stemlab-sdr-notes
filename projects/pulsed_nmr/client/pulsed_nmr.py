...
class PulsedNMR(QMainWindow, Ui_PulsedNMR):
  rates = {0:16.0, 1:32.0, 2:64.0, 3:128.0, 4:256.0, 5:512.0, 6:1024.0, 2048.0}
  def __init__(self):
    super(PulsedNMR, self).__init__()
    self.setupUi(self)
    self.rateValue.addItems(['16', '32', '64', '128', '256', '512', '1024', '2048'])
  ...
  def set_rate(self, index):
    ...
    time = np.linspace(0.0, (self.size - 1) / rate, self.size)
    # reset toolbar
    self.toolbar.home()
    ...
    self.canvas.draw()
    # set repetition time
    minimum = self.size / rate * 2.0
    if minimum < 80.0: minimum = 80.0
